 ...
 */
const SAFE_TREESHAKE_OPTIONS: RollupOptions["treeshake"] = {
  preset: "recommended",
...
function isProd(target: BuildTarget): boolean {
  return target.mode === "prod";
}

function createSwcPlugin(target: BuildTarget): Plugin | null {
...
function createTerserPlugin(target: BuildTarget): Plugin | null {
  if (!isProd(target)) return null;
...
function walkAst(node: AstNode, enter: (node: AstNode) => boolean | void): void {
...
function hasDirectEval(node: AstNode): boolean {
  ...
  walkAst(node, (current) => {
    ...
    if (current.type === "CallExpression") {
      ...
      if (isAstNode(callee) && callee.type === "Identifier" && callee.name === "eval") {
        found = true;
        return false;
      }
    }
...
function analyzeInlineableIIFE(fn: AstNode): IIFEAnalysis | null {
  ...
  walkAst(body, (node) => {
    ...
    if (node !== body) {
      ...
      if (isFunctionLikeNode(node) || node.type === "ClassExpression") return false;
...
function createParamPrefix(code: string, fn: AstNode, call: AstNode): string {
  ...
  const args = Array.isArray(call.arguments) ? (call.arguments as readonly AstNode[]) : [];
...
function replaceReturnsInBody(
  ...
  const orderedReturns = [...returns].sort((left, right) => left.start - right.start);
  let result = "";
...
function createInlineIIFEBodyBlock(
  ...
  const fallthrough = target.kind === "assign" ? `${target.target}=undefined;` : "";
...
function createUnusedIIFEStatementReplacement(
  ...
  const text = createInlineIIFEStatement(code, call, fn, { kind: "discard" }, index);
...
function createAssignmentIIFEStatementReplacement(
  ...
  const expression = statement.expression;
  if (!isAstNode(expression) || expression.type !== "AssignmentExpression") return null;
  ...
  const text = createAssigningInlineIIFEStatement(code, call, fn, target, index);
  if (text === null) return null;
...
function createVariableIIFEDeclarationReplacement(
  ...
  if (!isAstNode(declaration) || declaration.type !== "VariableDeclarator") return null;

  const id = declaration.id;
  const init = declaration.init;
  if (!isAstNode(id) || id.type !== "Identifier" || !isAstNode(init)) return null;
...
function safeInlineIIFEPlugin(): Plugin {
  return {
    ...
    renderChunk(code) {
      ...
      walkAst(ast, (node) => {
        ...
        if (node.type === "ExpressionStatement") {
          replacement =
            createAssignmentIIFEStatementReplacement(code, node, replacements.length) ??
            createUnusedIIFEStatementReplacement(code, node, replacements.length);
        } else if (node.type === "VariableDeclaration") {
          replacement = createVariableIIFEDeclarationReplacement(code, node, replacements.length);
        }
...
function validateFinalChunkPlugin(): Plugin {
  return {
...
function createPlugins(target: BuildTarget): Plugin[] {
  ...
  const swcPlugin = createSwcPlugin(target);
  if (swcPlugin !== null) plugins.push(swcPlugin);

  if (isProd(target)) plugins.push(safeInlineIIFEPlugin());

  const terserPlugin = createTerserPlugin(target);
  if (terserPlugin !== null) plugins.push(terserPlugin);
