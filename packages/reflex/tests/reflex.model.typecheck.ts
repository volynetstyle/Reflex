...
import { createModel, type ModelShape } from "../src/infra/model";
...
createModel((ctx) => ({
  count,
  doubled,
  inc: ctx.action(() => count.set((value) => value + 1)),
  nested: {
    reset: ctx.action(() => count.set(0)),
  },
}));
...
const createParamModel = createModel(
  (ctx, amount: number, label?: string, enabled: boolean = true) => ({
    ...
    nested: {
      ...
      bump: ctx.action(() => count.set((value) => value + amount)),
    },
  }),
);
